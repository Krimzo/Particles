...
Particles::Particles()
{
    ...
    const std::initializer_list<kl::dx::LayoutDescriptor> layout_descriptors = {
        ...
        {"KL_Position", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA,
         0},
        {"KL_Velocity", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA,
         0},
...
void Particles::update_camera()
{
    ...
    {
        const int2 mouse_delta = window.mouse.position() - start_mouse_position;
        camera_rotations.x = start_camera_rotations.x + mouse_delta.x * camera.sensitivity;
        camera_rotations.y =
            kl::clamp(start_camera_rotations.y + mouse_delta.y * camera.sensitivity, -VERTICAL_LIMIT, VERTICAL_LIMIT);
...
void Particles::compute_physics()
{
    ...
    {
        const float2 ndc = window.mouse.ndc_pos();
...
void Particles::render_ui()
{
    ...
    if (imgui::Begin("Scene"))
    {
        ...
        float3 background = camera.background;
        ...
        imgui::Text(
            kl::format("CPU Particle Count: ", cpu_size, " [", cpu_size * sizeof(Particle) * 1e-6, " MB]").c_str());
        imgui::Text(
            kl::format("GPU Particle Count: ", gpu_size, " [", gpu_size * sizeof(Particle) * 1e-6, " MB]").c_str());
        ...
        imgui::Text(kl::format("Mesh Triangle Count: ", triangle_count, " [",
                               triangle_count * sizeof(kl::Triangle) * 1e-6, " MB]")
                        .c_str());
        const size_t image_size = (size_t)selected_texture.width() * selected_texture.height();
        imgui::Text(kl::format("Texture Resolution: ", selected_texture.size(), " [",
                               image_size * sizeof(kl::RGB) * 1e-6, " MB]")
                        .c_str());
...
void Particles::generate_particle_mesh()
{
    ...
    else
    {
        for (kl::Triangle const& triangle : selected_mesh_triangles)
        {
            ...
            const float3 a_walk_direction = kl::normalize(triangle.c.position - triangle.a.position);
            const float3 b_walk_direction = kl::normalize(triangle.c.position - triangle.b.position);

            for (int i = 0; i <= step_count; i++)
            {
                const float3 a_walk_point = triangle.a.position + a_walk_direction * (i * generation_precision);
                const float3 b_walk_point = triangle.b.position + b_walk_direction * (i * generation_precision);
...
void Particles::generate_particle_line(kl::Triangle const& triangle, float3 const& start, float3 const& end)
{
    const float walk_distance = (end - start).length();
    const int step_count = int(walk_distance / generation_precision);
    const float3 walk_direction = kl::normalize(end - start);
    ...
    for (int i = 0; i <= step_count; i++)
    {
        ...
        const float3 weights = triangle.weights(particle.position);
...
void drag_float3(std::string_view const& text, float3& value, std::function<void()> const& callback, float width)
{
    ImGuiStyle& style = imgui::GetStyle();
    imgui::SetCursorPosY(imgui::GetCursorPosY() + style.FramePadding.y);
    imgui::Text(text.data());
